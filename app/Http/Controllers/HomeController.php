...
class HomeController extends Controller
{
    public function index(): Response
    {
        ...
        $lattestTotalDailyVaccinations = DailyVaccination::whereReferenceDate($lattestUpdateDatetime->reference_date)
            ->sum('day_total');

        // The number of vaccinated citizens for 1 day before the lattest date available
        $oneDayBeforeLattestTotalDailyVaccinations = DailyVaccination::whereReferenceDate($oneDayBeforelattestUpdateDatetime)
            ->sum('day_total');

        // The number of vaccinated citizens for the first dose for the lattest date available
        $lattestTotalDose1DailyVaccinations = DailyVaccination::whereReferenceDate($lattestUpdateDatetime->reference_date)
            ->sum('daily_dose_1');

        // The number of vaccinated citizens for the first dose for 1 day before the lattest date available
        $oneDayBeforeLattestTotalDose1DailyVaccinations = DailyVaccination::whereReferenceDate($oneDayBeforelattestUpdateDatetime)
            ->sum('daily_dose_1');

        // The number of vaccinated citizens for the second dose for the lattest date available
        $lattestTotalDose2DailyVaccinations = DailyVaccination::whereReferenceDate($lattestUpdateDatetime->reference_date)
            ->sum('daily_dose_2');

        // The number of vaccinated citizens for the first dose for 1 day before the lattest date available
        $oneDayBeforeLattestTotalDose2DailyVaccinations = DailyVaccination::whereReferenceDate($oneDayBeforelattestUpdateDatetime)
            ->sum('daily_dose_2');
